...
import { SessionController } from "../SpectacleSyncKit/Core/SessionController"; //TODO: fix path
...
export class LocationTracker extends BaseScriptComponent {
  ...
  onAwake() {
    ...
    if (SessionController.getIsReady()) {
      print("SessionController is already ready!");
      this.initializeLocationTracking();
      return;//don't call initializeLocationTracking twice
    }
    // if not ready, wait for SessionController to be ready before starting location tracking
    SessionController.notifyOnReady(() => {
      print("SessionController is finally ready!");
      this.initializeLocationTracking();
    });
  }
