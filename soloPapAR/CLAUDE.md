...
3. **`stakedBy != 0`**: The cell has someone's stake in it. Fire `playerDeathEvent` RPC with `vec3(stakedBy, clientID, getPlayerVisualID(stakedBy))` — killing whoever owns that stake. This fires *before* the self-check, so stepping back onto your **own** trail this life self-kills (`vec3(clientID, clientID, playerID)`) — **intended Paper.io self-collision**, a documented mechanic. Caveat for a future kill feed (FEAT-3): the self-collision payload shape `killerID === deadPlayerID` is identical to the voluntary-leave and out-of-bounds shapes, so the three causes need a distinct flag before they can be labeled. The third component is the victim's visualID for remote cleanup, see Death handling. Then, **only if the stake is another player's** (`stakedBy !== clientID`), also claim the cell for yourself: push to `stakeList`, update `localCellState`, and spawn a stake visual **immediately**, but **defer the cloud write 500 ms** so it lands *after* the killed player's death-clear (otherwise their clear would erase it). That deferred write captures `conversionEpoch` and **no-ops if the epoch changed** — i.e. if you looped back to your own claim (a conversion) or died within the 500 ms — so it can't revert a just-converted claim back to a stake, nor write a stake for a dead player (fixes former defect D3).
4. **`claimedBy == ID`**: Player stepped onto their own claimed territory. Call `addStakedRegionToClaim()` to convert the pending trail.
...
Grid coordinates (0–39 int)
     ...
     (present staker → death RPC             │
      vec3(dead, killer, visualID))          │
               │                                  │
               ▼                                  ▼
...
- **Self-collision death (intended mechanic)**: stepping back onto a cell you staked earlier this life kills you — Paper.io-correct self-collision, documented in the `sendData` decision tree (step 3). Known limitation for a future kill feed: the payload (`killerID === deadPlayerID`) is indistinguishable from a voluntary-leave / out-of-bounds death, so causes need a distinct flag before they can be labeled (see FEAT-3).
